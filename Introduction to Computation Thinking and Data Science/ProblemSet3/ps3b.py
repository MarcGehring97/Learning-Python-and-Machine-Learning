...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for trial in range(numTrials):
        ...
        for timestep in range(300):
            patient.update()
            yvals[timestep]=yvals[timestep]+patient.getTotalPop()
    for i in yvals:
        yvals[i]=yvals[i]/numTrials
    yvalsA=[]
    for i in yvals:
        yvalsA.append(yvals[i])
    pylab.plot(yvalsA,label="SimpleVirus")
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        for drug in activeDrugs:
            try:
                isResistantToAll=isResistantToAll and self.resistances[drug]
            except KeyError:
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    for trial in range(numTrials):
        ...
        for timestep in range(150):
            patient.update()
            yvalsA[timestep]=yvalsA[timestep]+patient.getTotalPop()
            if len(resistances)==0:
                yvalsB[timestep]=0
            else:
                yvalsB[timestep]=yvalsB[timestep]+patient.getResistPop(resistances)
        patient.addPrescription("guttagonol")
        for timestep in range(150, 300):
            patient.update()
            yvalsA[timestep]=yvalsA[timestep]+patient.getTotalPop()
            yvalsB[timestep]=yvalsB[timestep]+patient.getResistPop(patient.getPrescriptions())
    for i in yvalsA:
        yvalsA[i]=yvalsA[i]/numTrials
    yvalsAX=[]
    for i in yvalsA:
        yvalsAX.append(yvalsA[i])  
    for i in yvalsB:
        yvalsB[i]=yvalsB[i]/numTrials
    yvalsBX=[]
    for i in yvalsB:
        yvalsBX.append(yvalsB[i])
    pylab.plot(yvalsAX,label="TotalVirus")
    pylab.plot(yvalsBX,label="ResistantVirus")
